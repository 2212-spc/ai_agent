...
- **注册页面**：`frontend/register.html`
- **聊天界面**：`frontend/agent_chat.html`
...
2. 填写以下信息：
   - **用户名**：用于显示的名称（例如：zhangsan）
   - **邮箱**：作为登录账号（例如：user@example.com） ⚠️ **重要：必填且必须是有效邮箱格式**
   - **密码**：至少6个字符
   - **确认密码**：再次输入密码
3. 点击"注册"按钮
4. 注册成功后会显示绿色提示，3秒后自动跳转到登录页面

**登录账号：**
1. 访问 `http://localhost:5173/login`
2. 输入注册时使用的**邮箱**（不是用户名！）和**密码**
3. 点击"登录"按钮
4. 登录成功后自动跳转到聊天界面

**跳过登录：**
- 点击"跳过登录 →"按钮可以直接使用，无需注册账号
...
│
├── frontend-vue/               # Vue 3 前端（推荐）✨
│   ├── src/
│   │   ├── views/             # 页面组件
│   │   │   ├── AgentChat.vue      # 主聊天界面
│   │   │   ├── Login.vue          # 登录页面
│   │   │   ├── Register.vue       # 注册页面
│   │   │   ├── PromptManagement.vue  # Prompt管理
│   │   │   ├── KnowledgeBase.vue     # 知识库管理
│   │   │   ├── Memory.vue            # 记忆管理
│   │   │   └── ConversationHistory.vue  # 历史记录
│   │   ├── components/        # 公共组件
│   │   │   ├── Chat/          # 聊天组件
│   │   │   │   ├── ChatPanel.vue     # 聊天面板
│   │   │   │   └── TimelinePanel.vue # Timeline展示
│   │   │   └── Canvas/        # 画布组件
...
│   ├── login.html             # 登录页面
│   ├── register.html          # 注册页面
│   ├── agent_chat.html        # LangGraph Agent 界面
